...
def vis_logloss(eval_hist):
    ...
    """
    #Extract loss
    train_loss = eval_hist["train"]["logloss"]
    dev_loss = eval_hist["dev"]["logloss"]
    #Plot train dev loss
    sns.set_theme()
    plt.figure(figsize=(7, 4))
    plt.plot(train_loss, label="Train loss")
    plt.plot(dev_loss, label="Dev loss", linestyle="--")
    plt.xlabel("Boosting rounds")
    plt.ylabel("Log loss")
    plt.title("Trian and dev loss per boosing rounds")
    plt.legend()
    plt.show()
